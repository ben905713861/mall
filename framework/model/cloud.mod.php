 ...
 * WeEngine is NOT a free software, it under the license terms, visited http://www.mogela.cn/ for more details.
 */
defined('IN_IA') or exit('Access Denied');
define('APIHOST', 'http://v2.zhifun.cc/pull.php');
define('MODULE','wshcms');
function hasVerify(){
	global $_GPC;
	$result = ihttp_request(APIHOST,array('type' => 'checkauth','module' => MODULE,'website' => 1,'domain' => getDomain(),'code' => getVerifyCode()),null,5);
...
function getUserIP(){
	global $_GPC;
	$resp = ihttp_request(APIHOST,array('type'    => 'user','module'  => MODULE,'website' => 1,'domain'  => getDomain()),null,1);
...
function cloud_client_define() { //系统调用包含文件
	return array(
...
function _cloud_build_params() { //读取系统注册信息
	global $_W;
	$pars = array();
...
function _cloud_shipping_parse($dat, $file) { //检测系统版本及授权情况
	if (is_error($dat)) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系莫哥拉客服！');
	}
...
function cloud_prepare() { //加载注册信息对比
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		return error('-1', "您的站点只有在莫哥拉云服务平台成功注册后，才能使用云服务的相应功能。");
	}
	
	return true;
}

function cloud_build() { //检测系统版本信息
	$pars = _cloud_build_params();
	$pars['method'] = 'application.build2';
	$pars['extra'] = cloud_extra_account();
	$dat = cloud_request('http://v2.zhifun.cc/pull.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://v2.zhifun.cc', $pars, $headers, 300);
	if(is_error($dat)) {
